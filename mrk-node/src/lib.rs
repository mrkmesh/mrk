...
fn local_node_ledger_state(
    node_id: Option<u64>,
    registered_node: Option<&NodeRecord>,
) -> LocalNodeLedgerState {
    match node_id {
        None => LocalNodeLedgerState::Unregistered,
        Some(node_id) if registered_node.is_some() => LocalNodeLedgerState::Ready(node_id),
        Some(node_id) => LocalNodeLedgerState::Missing(node_id),
    }
}
...
pub enum DaemonCommand {
    ...
    Init {
        #[arg(long)]
        lite: bool,
        /// Consensus ledger ID; only selectable during initial data-directory setup.
        #[arg(long)]
...
#[derive(Subcommand, Serialize, Deserialize)]
pub enum NodePaymentPolicyCommand {
    Show {
        #[arg(long)]
...
pub enum BlockCommand {
    Status,
    /// List bootstrap checkpoints retained by this node.
    Checkpoints,
    Produce {
        #[arg(long)]
        allow_empty: bool,
...
pub enum GovernanceCommand {
    ...
    ProposeSet {
        ...
        #[arg(long)]
        title: String,
        #[arg(long, value_parser = governance_parameter_parser, required = true)]
        parameter: Vec<String>,
        #[arg(long, required = true)]
        value: Vec<String>,
        /// Epoch at whose boundary the complete parameter batch becomes active.
        #[arg(long)]
...
impl From<ProposalKind> for mrk_core::model::GovernanceProposalKind {
...
impl From<VoteChoice> for mrk_core::model::GovernanceVoteChoice {
...
fn governance_parameter_changes(
    parameters: Vec<String>,
    values: Vec<String>,
) -> Result<BTreeMap<String, String>> {
    if parameters.len() != values.len() {
        return Err(Error::msg(
            "each --parameter requires one corresponding --value",
        ));
    }
    let change_count = parameters.len();
    let changes = parameters
        .into_iter()
        .zip(values)
        .collect::<BTreeMap<_, _>>();
    if changes.len() != change_count {
        return Err(Error::msg(
            "a governance action cannot change the same parameter twice",
        ));
    }
    Ok(changes)
}
...
impl RelayHub {
    ...
    fn member_presence(
        ...
        network_alias: &str,
        observed_at: i64,
    ) -> Result<service::MemberPresenceListView> {
        let network = service::network_by_alias(paths, network_alias)?;
        let state = self
            .state
            .lock()
            .map_err(|_| Error::msg("relay connection state lock is poisoned"))?;
        let members = network
            ...
            .map(|member| {
                let connection_count = state
                    .member_connections
                    .get(&(network.network_id.clone(), member.member_id.clone()))
                    .map_or(0, Vec::len);
                service::MemberPresenceView {
            ...
            .collect();
        Ok(service::MemberPresenceListView {
            network: network.alias,
            network_id: network.network_id,
            relay_node_id: self
                .node_id
                .ok_or_else(|| Error::msg("Relay Node identity is unavailable"))?,
            observed_at,
            members,
        })
    }
        ...
        match frame.frame_type {
            ...
            FrameType::SenderCheckpoint => {
                ...
                if direction.awaiting_receipt
                    ...
                    || checkpoint.protocol_version != mrk_core::model::PROTOCOL_VERSION
            ...
            FrameType::ReceiverReceipt => {
                ...
                if receipt.receiver_member_id != expected_receiver
                    ...
                    || receipt.protocol_version != mrk_core::model::PROTOCOL_VERSION
...
fn record_auto_abandon_budget(
    paths: &DataPaths,
    authorization: &mrk_core::model::PaymentAuthorizationRecord,
    abandoned_bytes: u64,
    limit: u64,
    now: i64,
) -> bool {
...
fn execute_daemon_command(
    ...
    match command {
        ...
        DaemonCommand::Init { lite, ledger_id } => {
            let directory = paths.node_dir(&cli.node)?;
            if directory.exists() {
                return Err(Error::msg(format!("node '{}' already exists", cli.node)));
            }
            if let Some(ledger_id) = ledger_id.as_deref() {
                mrk_core::storage::validate_ledger_id(ledger_id)?;
            }
            ...
            let config = service::init_node_with_storage_mode_and_ledger_id(
                paths,
                &cli.node,
                &password,
                storage_mode,
            ...
            let ledger_id = paths.read_ledger()?.ledger_id;
            let mut value = serde_json::to_value(&config)?;
            value
                .as_object_mut()
                .expect("LocalNodeConfig serializes as an object")
                .insert("ledger_id".to_owned(), ledger_id.clone().into());
        ...
        DaemonCommand::Join {
            ...
            let node = service::join_node(
                ...
                &password,
                &endpoint,
                price_per_gib.as_deref(),
                Utc::now().timestamp(),
            )?;
            print_value(cli.output, &node, || {
                if let Some(previous_node_id) = node.previous_node_id {
                    format!(
                        "Previous Node ID: {previous_node_id}\nNew Node ID: {}\nStatus: {}\nEndpoint: {}\nIP slot: {}",
        ...
        DaemonCommand::UpdatePrice { price_per_gib } => {
            let password = read_password("Node Owner password: ")?;
            let (operation_id, node) = service::update_node_price(
                paths,
                &cli.node,
                &password,
                &price_per_gib,
...
pub fn execute_client_command(
    ...
    match command {
        ClientCommand::Block { command } => match command {
            ...
            BlockCommand::Checkpoints => {
                let checkpoints = service::bootstrap_checkpoints(paths)?;
                print_value(cli.output, &checkpoints, || {
                    if checkpoints.is_empty() {
                        "No retained bootstrap checkpoints.".to_owned()
                    } else {
                        let mut lines = vec!["HEIGHT\tFINALIZED AT\tSTATE ROOT".to_owned()];
                        lines.extend(checkpoints.iter().map(|checkpoint| {
                            format!(
                                "{}\t{}\t{}",
                                checkpoint.height, checkpoint.finalized_at, checkpoint.state_root
                            )
                        }));
                        lines.join("\n")
                    }
        ...
        ClientCommand::Validator { command } => match command {
            ...
            ValidatorCommand::Committee => {
                ...
                print_value(cli.output, &committee, || {
                    format!(
                        "Epoch: {}\nValidator set: {}\nActive: {:?}\nCandidates: {:?}\nQuorum: {}\nRotation: up to {} seats every {} Epoch(s)\nLast selection Epoch: {}\nNext scheduled rotation Epoch: {}\nNext height: {}\nRound: {}\nProposer: {}",
                        committee.epoch,
                        committee.validator_set_hash,
                        committee.active_validator_ids,
                        committee.candidate_node_ids,
                        committee.quorum,
                        committee.max_rotations_per_selection,
                        committee.rotation_interval_epochs,
                        committee
                            .last_selection_epoch
                            .map_or_else(|| "none".to_owned(), |value| value.to_string()),
                        committee.next_scheduled_rotation_epoch,
            ...
            TreasuryCommand::Status => {
                ...
                print_value(cli.output, &status, || {
                    format!(
                        ...
                        status.minimum_mature_governance_nodes,
                        status.active_validator_count,
                        status.minimum_active_validators,
        ...
        ClientCommand::Governance { command } => match command {
            GovernanceCommand::Status => {
                ...
                print_value(cli.output, &status, || {
                    format!(
                        ...
                        status.node1_direct_end_threshold,
            ...
            GovernanceCommand::BondStatus => {
                let status =
                    service::governance_bond_status(paths, &cli.node, Utc::now().timestamp())?;
                print_value(cli.output, &status, || {
                    format!(
                        "Node ID: {}\nGovernance eligible: {}\nGovernance Bond: {} / {}\nBonded at: {}\nMatures at: {}\nExit requested: {}\nBond unlock: {}",
                        status.node_id,
                        status.eligible,
                        status.governance_bond_display,
                        status.required_bond_display,
                        status
                            .bonded_at
                            .map_or_else(|| "not bonded".to_owned(), |value| value.to_string()),
                        status
                            .matures_at
                            .map_or_else(|| "not bonded".to_owned(), |value| value.to_string()),
                        status
            ...
            GovernanceCommand::Bond => {
                let password = read_password("Node keystore password: ")?;
                let receipt =
                    service::bond_governance(paths, &cli.node, &password, Utc::now().timestamp())?;
                print_value(cli.output, &receipt, || {
                    format!(
                        "SUBMITTED\nOperation: {}\nNode ID: {}\nBonded: {}\nTotal Governance Bond: {}\nMatures at: {}",
                        receipt.operation_id,
                        receipt.node_id,
                        receipt.bonded_display,
                        receipt.total_governance_bond_display,
                        receipt.matures_at,
                    )
                })?;
            }
            GovernanceCommand::Exit => {
                let password = read_password("Node Owner password: ")?;
                let operation_id = service::request_governance_exit(
                    paths,
                    &cli.node,
                    &password,
                    Utc::now().timestamp(),
            ...
            GovernanceCommand::WithdrawBond => {
                let password = read_password("Node Owner password: ")?;
                let (operation_id, amount) = service::withdraw_governance_bond(
                    paths,
                    &cli.node,
                    &password,
                    Utc::now().timestamp(),
            ...
            GovernanceCommand::ProposeSet {
                ...
                let changes = governance_parameter_changes(parameter, value)?;
                ...
                let proposal = service::create_governance_proposal(
                    ...
                    kind.into(),
                    &title,
                    mrk_core::model::GovernanceProposalAction::SetParameters {
                        changes,
                        effective_epoch,
                    },
                    Utc::now().timestamp(),
            ...
            GovernanceCommand::ProposePause {
                ...
                let proposal = service::create_governance_proposal(
                    ...
                    kind.into(),
                    &title,
                    mrk_core::model::GovernanceProposalAction::PauseEmission { reason },
                    Utc::now().timestamp(),
                )?;
                print_value(cli.output, &proposal, || {
            ...
            GovernanceCommand::ProposeResume { kind, title } => {
                ...
                let proposal = service::create_governance_proposal(
                    ...
                    kind.into(),
                    &title,
                    mrk_core::model::GovernanceProposalAction::ResumeEmission,
                    Utc::now().timestamp(),
                )?;
                print_value(cli.output, &proposal, || {
                    format!("PROPOSAL_CREATED\nID: {}", proposal.proposal_id)
            ...
            GovernanceCommand::ProposeTreasurySpend {
                ...
                let proposal = service::create_governance_proposal(
                    ...
                    &cli.node,
                    &password,
                    mrk_core::model::GovernanceProposalKind::Critical,
                    &title,
                    mrk_core::model::GovernanceProposalAction::TreasurySpend {
                        recipient: to,
                        amount,
            ...
            GovernanceCommand::Set {
                ...
                let changes = governance_parameter_changes(parameter, value)?;
                let password = read_password("Genesis Node Owner password: ")?;
                let receipt = service::governance_set_parameters_at_epoch(
                    paths,
                    &cli.node,
                    &password,
                    changes,
                ...
                print_value(cli.output, &receipt, || {
                    format!(
                        ...
                        receipt.operation_id,
                        receipt.action,
                        receipt.payload["epoch_context_activation"],
                        receipt.payload["changes"]
                    )
                })?;
            }
...
fn run_node_server(
    ...
    if paths.read_node_config(name)?.node_id.is_none() {
        println!(
            "Node has not joined; keep this process running and use 'mrk node join' from another shell."
        );
    }
    loop {
        ...
        if !public_sync_worker_started && config.bootstrap_peer.is_some() {
        ...
        let registered_node = config
            .node_id
            .map(|node_id| service::node_record_by_id_if_present(paths, node_id))
            .transpose()?
            .flatten();
        let node_id = match local_node_ledger_state(config.node_id, registered_node.as_ref()) {
            LocalNodeLedgerState::Unregistered => {
                thread::sleep(Duration::from_millis(100));
                continue;
            }
            LocalNodeLedgerState::Missing(node_id) if config.bootstrap_peer.is_some() => {
                if !ledger_recovery_waiting {
                    println!(
                        "Local Ledger does not contain registered Node {node_id}; waiting for trusted bootstrap recovery."
                    );
                    println!(
                        "Install a trusted checkpoint with 'mrk node bootstrap --peer <peer> --checkpoint-height <height> --checkpoint-root <state_root>'."
                    );
                    ledger_recovery_waiting = true;
                }
                thread::sleep(Duration::from_millis(100));
                continue;
            }
            LocalNodeLedgerState::Missing(node_id) => {
                return Err(Error::msg(format!(
                    "local Ledger does not contain registered Node {node_id}; restore chain.redb or a verified Ledger backup"
                )));
            }
            LocalNodeLedgerState::Ready(node_id) => node_id,
        };
        if ledger_recovery_waiting {
            println!("Local Node registration recovered from the synchronized Ledger.");
            ledger_recovery_waiting = false;
        }
        let registered_node = registered_node.expect("local Node ledger state was checked");
        if registered_node.status == NodeStatus::Exited {
            if !exited_node_waiting {
                println!(
                    "Node {node_id} is EXITED; the administration socket remains available for 'mrk node join'."
                );
                exited_node_waiting = true;
            }
            thread::sleep(Duration::from_millis(100));
            continue;
        }
        if registered_node.status == NodeStatus::Suspended {
            return Err(Error::msg("node cannot run while status is SUSPENDED"));
        }
        if exited_node_waiting {
            println!("Owner joined as replacement Node {node_id}.");
            exited_node_waiting = false;
        }
        ...
        if public_listener.is_none() {
            ...
            println!(
                "Relay: /v1/relay  RPC: /v1/rpc  Explorer: /explorer  Consensus: /v1/consensus  Health: /health  Ready: /ready  Probe: /v1/probe?challenge=<random>"
            );
        }
...
fn flush_one_traffic_settlement(
    ...
    if let Some(operation_id) = &pending.submission_operation_id {
        ...
        {
            Some(mrk_core::model::OperationStatus::Finalized) => {
                return paths.remove_pending_traffic_settlement_if_not_newer(
                    &authorization_id,
                    direction,
                    pending_sequence,
            ...
            Some(mrk_core::model::OperationStatus::Pending) => return Ok(()),
...
fn handle_connection(
    ...
    let method = request_parts
        .next()
        .ok_or_else(|| Error::msg("invalid HTTP request method"))?;
    let target = request_parts
        .next()
        .ok_or_else(|| Error::msg("invalid HTTP request target"))?;
    if target == "/v1/relay" {
    ...
    if target == "/v1/rpc" {
        let response =
            websocket_server_response_for_protocol(request, "/v1/rpc", rpc::RPC_PROTOCOL)?;
        stream.write_all(response.as_bytes())?;
        stream.flush()?;
        return handle_rpc_connection(paths, stream, hub);
    }
    if target == explorer::EXPLORER_BASE_PATH || target.starts_with("/explorer/") {
        if !matches!(method, "GET" | "HEAD") {
            let response = "HTTP/1.1 405 Method Not Allowed\r\nAllow: GET, HEAD\r\nContent-Length: 0\r\nConnection: close\r\n\r\n";
            stream.write_all(response.as_bytes())?;
            return Ok(());
        }
        let Some(asset) = explorer::asset(target) else {
...
fn handle_rpc_connection(
    ...
    loop {
        ...
        let response = match serde_json::from_slice::<rpc::Request>(&bytes) {
            Ok(request) => {
                let id = request.id;
                ...
                {
                    rpc::Response::error(id, "RATE_LIMITED", "RPC connection rate limit exceeded")
                } else {
                    match execute_public_rpc(paths, &hub, request) {
                        Ok(result) => rpc::Response::success(id, result),
                        Err(error) => {
                            rpc::Response::error(id, "INVALID_REQUEST", error.to_string())
                        }
                    }
                }
            }
            Err(error) => rpc::Response::error(0, "INVALID_JSON", error.to_string()),
        };
        let mut response_bytes = serde_json::to_vec(&response)?;
        if response_bytes.len() > MAX_CONSENSUS_MESSAGE_SIZE {
            response_bytes = serde_json::to_vec(&rpc::Response::error(
                response.id,
                "RESPONSE_TOO_LARGE",
                "RPC response exceeds 16 MiB; use a newer checkpoint or smaller query",
            ))?;
        }
...
fn execute_public_rpc(
    paths: &DataPaths,
    hub: &RelayHub,
    request: rpc::Request,
) -> Result<serde_json::Value> {
    if request.method == "system.ping" {
        return Ok(attach_node_version(rpc::execute(paths, request)?));
    }
    if request.method != "member.list" {
        return rpc::execute(paths, request);
    }
    let network = request
        .params
        .get("network")
        .and_then(serde_json::Value::as_str)
        .ok_or_else(|| Error::msg("missing or invalid 'network' parameter"))?;
    serde_json::to_value(hub.member_presence(paths, network, Utc::now().timestamp())?)
        .map_err(Into::into)
}

fn attach_node_version(mut result: serde_json::Value) -> serde_json::Value {
    result["node_version"] = serde_json::json!(env!("CARGO_PKG_VERSION"));
    result
}
...
fn governance_proposal_text(
    proposal: &mrk_core::model::GovernanceProposalRecord,
...
mod tests {
    ...
    #[test]
    fn public_system_info_reports_the_node_binary_version() {
        let result = attach_node_version(serde_json::json!({ "protocol": rpc::RPC_PROTOCOL }));

        assert_eq!(result["node_version"], env!("CARGO_PKG_VERSION"));
    }

    #[test]
    fn registered_node_waits_until_its_ledger_record_is_restored() {
        assert_eq!(
            local_node_ledger_state(None, None),
            LocalNodeLedgerState::Unregistered
        );
        assert_eq!(
            local_node_ledger_state(Some(7), None),
            LocalNodeLedgerState::Missing(7)
        );
    }
    ...
    fn relay_node_requests_checkpoint_when_time_window_expires() {
        ...
        let now = Utc::now().timestamp();
        let authorization = mrk_core::model::PaymentAuthorizationRecord {
            ...
            settled_amount: 0,
            traffic_protocol_fee_bps: 100,
            traffic_treasury_share_bps: 5_000,
