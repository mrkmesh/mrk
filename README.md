...
Payment checkpoints are pipelined rather than acting as a synchronous traffic barrier. The default window is 30 seconds or 32 MiB. At a boundary the Node requests a signed checkpoint for the latest prefix while the client continues into one additional window. The Node persists the receiver's receipt before considering that prefix paid. A direction is backpressured only if the receipt is still outstanding when the second window reaches its boundary, and the absolute unreceipted allowance never exceeds two configured windows (64 MiB with the default). Receipt arrival advances only to the billed prefix, leaving any bytes already sent in the second window unreceipted until the next checkpoint.

Each Node publishes a revisioned payment-window capability in the on-chain registry. The byte window may be 1–64 MiB and the time window 5–300 seconds; `mrk node update-relay-capabilities --payment-window-bytes <BYTES> --payment-window-seconds <SECONDS>` signs the next capability revision. `WELCOME` advertises the same revision and values, and clients require them to match the finalized registry before reserving a session. `PaymentAuthorization` snapshots the revision and values, so an update affects only new authorizations and existing sessions keep their negotiated limits.

Pressing Ctrl+C starts a graceful close instead of terminating immediately. Each endpoint sends an authenticated FIN and a `CloseIntent`; the Node answers with a final `CheckpointRequest`, and the clients return the ordinary dual-signed final receipt. The CLI exits after the Node has persisted those receipts. The Node then finalizes settlement and releases the unused reservation in the background. A second Ctrl+C forces shutdown if the peer cannot complete the receipt exchange. `mrk node run` applies the same rule to SIGINT and SIGTERM: it stops accepting new Relay channels, drains every active channel at its current payment boundary, and exits after the final receipts are persisted. A second signal forces immediate exit, and a drain that cannot finish is bounded by a 30-second timeout.

If a client process or connection disappears before that exchange, the running Node keeps the unsigned tail only in memory and records a small authorization hold without DATA, sequence counters, or transcript state. `mrk pipe --max-auto-recovery-bytes N` first recovers matching interrupted sessions and only then opens a new session; the standalone `payment settle` command exposes the same limit for manual recovery. The default is zero. The recovery channel carries settlement frames only, and the Node's bounded local auto-abandon policy still applies to a tail retained by the running process. After an unclean Node exit, the exact tail no longer exists and is deliberately treated as lost: on the next startup the Node submits an Owner-signed abandon operation for that hold instead of blocking the Members on an unrecoverable session. Dual-signed receipts awaiting chain settlement remain persisted and are excluded from abandonment until settlement reaches finality.
