...
pub(crate) fn list(read: &ReadTransaction) -> Result<Vec<(u64, LedgerState)>> {
    let table = read.open_table(CHECKPOINTS_TABLE).map_err(redb_error)?;
    let mut entries = table.iter().map_err(redb_error)?;
    let mut checkpoints = Vec::new();
    while let Some(entry) = entries.next_back() {
        let (height, bytes) = entry.map_err(redb_error)?;
        checkpoints.push((height.value(), serde_json::from_slice(bytes.value())?));
    }
    Ok(checkpoints)
}
