...
pub(crate) fn operation_by_id(
    write: &WriteTransaction,
    operation_id: &str,
) -> Result<Option<OperationRecord>> {
    operation::get(write, operation_id)
}

pub(crate) fn operations_by_ids(
    write: &WriteTransaction,
...
pub(crate) fn blocks_descending(
    write: &WriteTransaction,
    pruned_through_height: u64,
    before_height: Option<u64>,
    limit: usize,
) -> Result<(Vec<BlockRecord>, Option<u64>)> {
    let Some(tip) = block::tip(write)? else {
        return Ok((Vec::new(), None));
    };
    let mut height = before_height
        .map(|cursor| cursor.saturating_sub(1))
        .unwrap_or(tip.height)
        .min(tip.height);
    let first_retained = pruned_through_height.saturating_add(1);
    let mut blocks = Vec::with_capacity(limit);
    while height >= first_retained && blocks.len() < limit {
        blocks.push(
    ...
    let next_cursor = blocks
        .last()
        .filter(|block| block.height > first_retained)
...
pub(crate) fn persist_histories(
    ...
    state::persist(write, before, after)?;
...
pub(crate) fn persisted_state(ledger: &LedgerState) -> LedgerState {
    ...
    state.network_aliases.clear();
    state.nodes.clear();
    state.ip_slots.clear();
    state.availability_slots.clear();
    state.payment_authorizations.clear();
    state.treasury_spends.clear();
    state.governance.actions.clear();
    state.governance.proposals.clear();
    state.consensus.double_sign_evidence.clear();
    state.finalized_checkpoint = None;
    state
}
