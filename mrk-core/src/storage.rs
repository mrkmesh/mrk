...
impl DataPaths {
    ...
    pub fn initialize_ledger(&self, requested_ledger_id: Option<&str>) -> Result<String> {
        if let Some(ledger_id) = requested_ledger_id {
        ...
        let ledger_id = {
            let mut table = write.open_table(LEDGER_TABLE).map_err(redb_error)?;
            if let Some(bytes) = table.get(LEDGER_STATE_KEY).map_err(redb_error)? {
                let existing: LedgerState = serde_json::from_slice(bytes.value())?;
                if requested_ledger_id.is_some_and(|ledger_id| existing.ledger_id != ledger_id) {
                    return Err(Error::msg(format!(
                        "ledger is already initialized as '{}'; it cannot be renamed to '{}'",
                        existing.ledger_id,
                        requested_ledger_id.expect("checked as present")
                    )));
                }
                existing.ledger_id
            } else {
                let ledger_id = requested_ledger_id.unwrap_or(crate::model::DEFAULT_LEDGER_ID);
    ...
    pub(crate) fn with_active_ledger_mut<T>(
        ...
        if let Some(snapshot) =
            store::account_ranking::snapshot_for_finalized_transition(&before, &ledger)?
        {
            store::account_ranking::store(&write, &snapshot)?;
        }
        {
    ...
    pub(crate) fn consensus_catch_up_history(
        ...
        store::initialize_tables(&write)?;
        let mut ledger = load_persisted_ledger(&write)?;
        store::hydrate_state(&write, &mut ledger)?;
        let history =
    ...
    pub(crate) fn stored_blocks_descending(
        &self,
        before_height: Option<u64>,
        limit: usize,
    ) -> Result<(Vec<crate::model::BlockRecord>, Option<u64>)> {
        let db = self.open_chain_db()?;
        let write = db.begin_write().map_err(redb_error)?;
        store::initialize_tables(&write)?;
        let pruned_through_height = load_persisted_ledger(&write)?.pruned_through_height;
        let blocks = store::blocks_descending(&write, pruned_through_height, before_height, limit)?;
        write.commit().map_err(redb_error)?;
        Ok(blocks)
    }
    ...
    pub(crate) fn stored_operation(
        &self,
        operation_id: &str,
    ) -> Result<crate::model::OperationRecord> {
        let db = self.open_chain_db()?;
        let write = db.begin_write().map_err(redb_error)?;
        store::initialize_tables(&write)?;
        let operation = store::operation_by_id(&write, operation_id)?
            .ok_or_else(|| Error::msg(format!("operation {operation_id} does not exist")))?;
        write.commit().map_err(redb_error)?;
        Ok(operation)
    }

    pub(crate) fn stored_operations(
        &self,
        operation_ids: &[String],
    ) -> Result<Vec<crate::model::OperationRecord>> {
        let db = self.open_chain_db()?;
        let write = db.begin_write().map_err(redb_error)?;
        store::initialize_tables(&write)?;
        let operations = store::operations_by_ids(&write, operation_ids)?;
        write.commit().map_err(redb_error)?;
        Ok(operations)
    }
    ...
    pub(crate) fn account_ranking_snapshot(
        &self,
    ) -> Result<(
        String,
        u64,
        Option<store::account_ranking::AccountRankingSnapshot>,
    )> {
        let db = self.open_chain_db()?;
        let read = db.begin_read().map_err(redb_error)?;
        let (ledger_id, epoch) = {
            let table = read.open_table(LEDGER_TABLE).map_err(redb_error)?;
            let bytes = table
                .get(LEDGER_STATE_KEY)
                .map_err(redb_error)?
                .ok_or_else(|| Error::msg("ledger state is missing"))?;
            let ledger: LedgerState = serde_json::from_slice(bytes.value())?;
            (ledger.ledger_id, ledger.epoch_number)
        };
        Ok((ledger_id, epoch, store::account_ranking::latest(&read)?))
    }
    ...
    pub fn bootstrap_checkpoints(&self) -> Result<Vec<(u64, LedgerState)>> {
        let db = self.open_chain_db()?;
        let read = db.begin_read().map_err(redb_error)?;
        store::checkpoint::list(&read)
    }
...
pub fn validate_ledger_id(ledger_id: &str) -> Result<()> {
    ...
    {
        return Err(Error::msg(
            "ledger ID must be 3 to 64 lowercase ASCII letters, digits, or '-' and must start and end with a letter or digit",
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn entity_tables_restore_current_state_and_sparse_finalized_overlay() {
        ...
        paths
            .with_ledger_mut(|ledger| {
                ...
                ledger.accounts.insert(
                    "pending-only".to_owned(),
                    crate::model::AccountState {
                        balance: 3,
                        ..Default::default()
                    },
                );
                let mut finalized = ledger.clone();
                finalized.accounts.get_mut("alice").unwrap().balance = 7;
                finalized.accounts.remove("pending-only");
                finalized.accounts.insert(
                    "finalized-only".to_owned(),
                    crate::model::AccountState {
                        balance: 5,
                        ..Default::default()
                    },
                );
                finalized.finalized_checkpoint = None;
                ledger.finalized_checkpoint = Some(Box::new(finalized));
                Ok(())
            })
            .unwrap();

        let restored = paths.read_ledger().unwrap();
        assert_eq!(restored.accounts["alice"].balance, 10);
        assert!(restored.accounts.contains_key("pending-only"));
        assert!(!restored.accounts.contains_key("finalized-only"));
        let finalized = restored.finalized_checkpoint.unwrap();
        assert_eq!(finalized.accounts["alice"].balance, 7);
        assert!(!finalized.accounts.contains_key("pending-only"));
        assert_eq!(finalized.accounts["finalized-only"].balance, 5);
    }
