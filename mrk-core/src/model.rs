...
pub const FEE_MULTIPLIER_BPS: u32 = 10_000;
/// Maximum interval between incremental reward vests.
pub const REWARD_VESTING_STEP_SECONDS: i64 = 86_400;
/// Maximum precision of the reward vesting table.
pub const REWARD_VESTING_QUANTIZATION_SECONDS: i64 = 12 * 60 * 60;
...
pub const AVAILABILITY_FINALITY_GRACE_SECONDS: i64 = 30;
...
fn default_required_governance_bond() -> u128 {
    10_000 * MRK_SCALE
}

fn default_governance_bond_maturity_seconds() -> i64 {
    60 * 86_400
}

fn default_governance_bond_unlock_seconds() -> i64 {
    30 * 86_400
}
...
fn default_fee_multiplier_bps() -> u32 {
    FEE_MULTIPLIER_BPS
}
...
pub struct FeePolicy {
    pub version: u64,
    pub base_fee_per_unit: u128,
    pub min_multiplier_bps: u32,
    pub max_multiplier_bps: u32,
    pub target_units_per_epoch: u64,
    pub max_units_per_block: u64,
    pub adjustment_denominator: u32,
    pub traffic_protocol_fee_bps: u32,
    pub traffic_treasury_share_bps: u32,
}
...
impl Default for FeePolicy {
    fn default() -> Self {
        Self {
            version: 1,
            base_fee_per_unit: TRANSFER_FEE,
            min_multiplier_bps: 2_500,
            max_multiplier_bps: 100_000,
            target_units_per_epoch: 900_000,
            max_units_per_block: 10_000,
...
fn default_validator_rotation_interval_epochs() -> u32 {
    1
}
...
pub struct LedgerSettings {
    ...
    #[serde(default = "default_required_governance_bond")]
    pub required_governance_bond: u128,
    #[serde(default = "default_governance_bond_maturity_seconds")]
    pub governance_bond_maturity_seconds: i64,
    #[serde(default = "default_governance_bond_unlock_seconds")]
    pub governance_bond_unlock_seconds: i64,
    ...
    #[serde(default = "default_validator_rotation_interval_epochs")]
    pub validator_rotation_interval_epochs: u32,
...
impl Default for LedgerSettings {
    fn default() -> Self {
        Self {
            ...
            ip_reuse_cooldown_seconds: 0,
            governance_min_service_seconds: default_governance_min_service_seconds(),
            required_governance_bond: default_required_governance_bond(),
            governance_bond_maturity_seconds: default_governance_bond_maturity_seconds(),
            governance_bond_unlock_seconds: default_governance_bond_unlock_seconds(),
            ...
            validator_rotation_interval_epochs: default_validator_rotation_interval_epochs(),
...
pub struct EpochContext {
    ...
    pub validator_bonus_ids: Vec<u64>,
    pub node1_single_producer: bool,
}
...
pub struct LedgerState {
    ...
    #[serde(default)]
    pub fee_units_used_in_epoch: u64,
    #[serde(default)]
    ...
    pub epoch_contexts: BTreeMap<u64, EpochContext>,
    pub settings: LedgerSettings,
...
impl Default for LedgerState {
    fn default() -> Self {
        ...
        let epoch_ended_at = now + settings.epoch_seconds;
        let epoch_contexts = BTreeMap::from([(
            0,
            EpochContext {
                epoch: 0,
                started_at: now,
                ended_at: epoch_ended_at,
                submission_deadline: epoch_ended_at + AVAILABILITY_FINALITY_GRACE_SECONDS,
        ...
        Self {
            ...
            epoch_contexts,
            settings,
            scheduled_parameter_changes: BTreeMap::new(),
...
pub const DEFAULT_RELAY_PRICE_PER_GIB: u128 = 10 * MRK_SCALE;
...
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq)]
/// Node reward tranche aggregated with every other tranche at the same unlock time.
pub struct RewardVestingBucket {
    pub unlock_at: i64,
    pub amount: u128,
}
...
pub struct NodeRecord {
    ...
    pub eligible_seconds_by_epoch: BTreeMap<u64, u64>,
    ...
    /// Sorted by `unlock_at`, with at most one bucket for each timestamp.
    #[serde(default)]
    pub reward_vesting_buckets: Vec<RewardVestingBucket>,
    pub validator: bool,
...
mod tests {
    ...
    fn epoch_defaults_to_thirty_minutes_and_five_hundred_mrk() {
        ...
        assert_eq!(ledger.reward_immediate_bps_snapshot, 2_500);
        ...
        assert_eq!(settings.ip_reuse_cooldown_seconds, 0);
        assert_eq!(settings.required_service_bond, 500 * super::MRK_SCALE);
        assert_eq!(settings.required_governance_bond, 10_000 * super::MRK_SCALE);
