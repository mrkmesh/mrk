...
const STREAM_E2E_PROTOCOL: &str = "mrk.stream.e2e.v1";
...
impl MemberIdentity {
    ...
    pub async fn from_relay(
        ...
        let mut rpc_endpoint =
            mrk_core::endpoint::normalize_websocket_url(endpoint, mrk_core::endpoint::RELAY_PATH)
                .map_err(|error| RelayError::InvalidConfig(error.to_string()))?;
        rpc_endpoint.set_path(mrk_core::endpoint::RPC_PATH);
...
impl RelayConnection {
    ...
    fn rpc_endpoint(&self) -> Result<url::Url, RelayError> {
        let mut endpoint = mrk_core::endpoint::normalize_websocket_url(
            &self.inner.options.endpoint,
            mrk_core::endpoint::RELAY_PATH,
        )
        .map_err(|error| RelayError::InvalidConfig(error.to_string()))?;
        endpoint.set_path(mrk_core::endpoint::RPC_PATH);
    ...
    async fn reserve_member_session(
        ...
        let network: NetworkRecord = serde_json::from_value(network_value)
            .map_err(|error| RelayError::Authorization(error.to_string()))?;
        let relay: service::RegistryNodeView = serde_json::from_value(relay_value)
            .map_err(|error| RelayError::Authorization(error.to_string()))?;
        ...
        let expected_price_per_gib = relay
        ...
        let fee_quote = self
            ...
            .await?;
        let max_fee_base_units = fee_quote
            .get("recommended_max_fee")
        ...
        let fee_policy_version = fee_quote
            .get("policy_version")
            .and_then(serde_json::Value::as_u64)
            .ok_or_else(|| RelayError::Authorization("invalid fee policy version".to_owned()))?;
...
fn frame_error(payload: &[u8]) -> RelayError {
    let message = serde_json::from_slice::<mrk_core::relay::ErrorPayload>(payload)
...
impl StreamCipher {
    fn derive(shared_secret: &[u8], context: Vec<u8>, initiator: bool) -> Result<Self, RelayError> {
        ...
        let info = [STREAM_E2E_PROTOCOL.as_bytes(), context.as_slice()];
        let okm = prk
...
async fn establish_channel_parts(
    ...
    let (peer_ephemeral_key, offer_bytes, response_bytes) = if initiator {
        let mut offer = KeyOffer {
            protocol: STREAM_E2E_PROTOCOL.to_owned(),
            channel_id: transport.channel_id,
        ...
        let mut response = KeyResponse {
            ...
            offer_hash: sha256_full_id("stream-offer", &offer_bytes),
...
fn validate_key_offer(
    ...
    if offer.protocol != STREAM_E2E_PROTOCOL
...
fn validate_key_response(
    ...
    if response.protocol != STREAM_E2E_PROTOCOL
        ...
        || response.offer_hash != sha256_full_id("stream-offer", offer_bytes)
...
fn encryption_context(
    ...
    serde_json::to_vec(&serde_json::json!({
        ...
        "handshake_transcript_hash": sha256_full_id("stream-handshake", &transcript),
    }))
    .map_err(|error| RelayError::Protocol(error.to_string()))
}
